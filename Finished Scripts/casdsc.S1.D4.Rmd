...
sort(myCat, decreasing = T)

# Make a second concatenation and combine it with the first.
myCat2 <- c(6,7,35,45)

myCat3 <- c(myCat, myCat2)

myList1 <- list(1,2,3,4,5)
myList2 <- list(6,7,8,9)
...
secondDict <- c("hat"=6.56,"scarfs"=3257)

as.numeric(firstDict["hat"] * secondDict["hat"])
...
c <- seq(0,10, by=1.2) #try changing the by value
c
...
A$sum <- A$pial1a+A$ideo
summary(A$sum)

A$sum2 <- rowSums(A[,c("pial1a","ideo")], na.rm = T)
summary(A$sum2)
# Remove a column.
A1 <- A[-6]
...
table(df$company)
df1 <- subset(df, company == "FORD WERKE GMBH")

# Create a new dataset of just Volvo cars.
volvo <- subset(df, company == "VOLVO")

# Create a new dataset that is just DIESEL cars.
diesel <- subset(df, fuel == "DIESEL")

# What is the mean of OB fuel for just PETROL/ELECTRIC cars?
mean(df$ob.fuel[df$fuel=="PETROL/ELECTRIC"])
pe <- subset(df, fuel == "PETROL/ELECTRIC")
mean(pe$ob.fuel)
```
